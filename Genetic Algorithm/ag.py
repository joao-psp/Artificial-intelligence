...
class Ag:
    ...
    def gerarGrafico(self, tempo, minimo, media, maximo):
        plt.plot(tempo, minimo, label='Minimo')
        plt.plot(tempo, media, label='Media')
        plt.plot(tempo, maximo, label='Maximo')
        plt.ylabel('Aptidao')
        plt.xlabel('Iteraçao')
        plt.title('Grafico Aptidao x Iteracao')
        plt.legend()
        plt.show()
    ...
    def melhoresCromossomos(self):
        n_melhores = 3
        melhores = sorted(self.populacao, reverse=False, key = lambda Cromossomo: Cromossomo.aptidao)
    ...
    def executar(self, elitismo = False):
        ...
        print(self.populacao[0].alelos[0])
        print(self.populacao[0].alelos[1])
